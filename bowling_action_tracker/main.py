...
# # Function to smooth points using a rolling average
# def smooth_trajectory(points, smoothing_window=10):
#     if len(points) < smoothing_window:
#         return points
#     smoothed = []
#     for i in range(len(points)):
#         start_idx = max(0, i - smoothing_window + 1)
#         window = points[start_idx:i + 1]
...
#         # Smooth the trajectory points
#         smoothed_positions = smooth_trajectory(wrist_positions)

#         # Draw right-hand keypoints
#         cv2.circle(frame, shoulder_coords, 10, (0, 255, 0), -1)  # Shoulder
#         cv2.circle(frame, elbow_coords, 10, (0, 255, 255), -1)   # Elbow
#         cv2.circle(frame, wrist_coords, 10, (255, 0, 0), -1)     # Wrist

#         # Draw line between elbow and wrist
#         cv2.line(frame, elbow_coords, wrist_coords, (255, 0, 255), thickness=15)

#         # Draw smooth trajectory line for the wrist
#         if len(smoothed_positions) > 1:
#             cv2.polylines(frame, [np.array(smoothed_positions, dtype=np.int32)], isClosed=False, color=(0, 0, 255), thickness=15)
